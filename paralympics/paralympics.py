...
def region_NOC(NOC):
    ...
    return regions_schema.dump(region)
...
def add_region():
    ...
    db.session.commit()
    return {"message": f"Region added with id= {region.NOC}"}
...
def delete_region(NOC):
    ...
    db.session.commit()
    return {"message": f"Region deleted with id= {NOC}"}
